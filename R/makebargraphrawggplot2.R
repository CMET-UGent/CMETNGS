...
#'  (to be implemented)
#' @param samples is a string for the title of the generated bargraph
                                 ...
                                 plot=TRUE,...)
{
  ...
  if(shared.abs==T)
  {
    ...
    shared<-sweep(shared,MARGIN=2,as.numeric(effectivereads),'*') #from the Ramiro Era
  }

  seltax.str<-paste(as.character(bquote(tax)),taxlevel,sep="$")
  seltax.val<-eval(parse(text=seltax.str))
  unclass.val<-as.matrix(seltax.val) #variable with highest classifiable taxon of unclassifieds
  #raw shared file, tax not preprocessed yet, auto-assign unclassified to closest taxlevel
  ...
  if(length(unclass.index)!=0)
  {
    for(i in 1:length(unclass.index))
    {
      ...
      class.colidx<-min(grep("unclassified",as.character(as.matrix(tax[unclass.rowidx,]))))-1 #column index of deepest assigned taxon
      ...
      {
        unclass.val[unclass.index[i]]<-paste("unclassified ",as.matrix(tax)[unclass.index[i],class.colidx],sep="")
  ...
  if(length(incert.index)!=0)
  {
    for(i in 1:length(incert.index))
    {
      ...
      class.colidx<-min(grep("Incertae",as.character(as.matrix(tax[incert.rowidx,]))))-1
      #column index of deepest assigned taxon
      if(class.colidx>=1) #maybe consider other threshold so that for summing e.g. phyla are allowed
      {
        unclass.val[incert.index[i]]<-paste("unclassified ",as.matrix(tax)[incert.index[i],class.colidx],sep="")
  ...
  if(tax.prob==F)
  {
    ...
    seltax.val[grep("unclassified",seltax.val)]<-paste(rep("unclassified",length(grep("unclassified",seltax.val))),seq(1,length(grep("unclassified",seltax.val))),sep="")
    seltax.val[grep("Incertae",seltax.val)]<-paste(rep("Incertae",length(grep("Incertae",seltax.val))),seq(1,length(grep("Incertae",seltax.val))),sep="")
    ...
    {
      shared.mfsum[i,]<-colSums(shared.tax[which(shared.tax$seltax.val==multtax[i]),1:ncol(shared.tax)-1])
    }
    shared.mfsum.tax<-data.frame(shared.mfsum,multtax) #contains all summed sequences of all duplicated or more assigned taxa
    shared.tax.single<-shared.tax[shared.tax$seltax.val%in%setdiff(shared.tax$seltax.val,shared.mfsum.tax$multtax),] #select all single occurences
    names(shared.mfsum.tax)<-names(shared.tax.single) #make sure that names are the same for rbind command
  ...
  }else
  {
    ...
    # the next command clearly illustrates the issue: levels(as.factor(as.character(seltax.val[duplicated(seltax.val)])))
    # use the data without the probabilities for the summing sub("\\(.*\\)","",seltax.val)
    # new proposal: everything above a certain bootstrap value gets summed (10000 Wang iterations)

    # make df to do thresholding upon
    selframe<-data.frame(do.call(rbind,strsplit(sub("\\(","\\.\\(",as.character(levels(seltax.val))),"\\.")))

    selframe<-cbind(selframe,as.character(levels(unclass.val))) #as.character(levels(seltax.val))
    names(selframe)<-c("taxon","bootstrap","full")

    selframe$bootstrap<-as.numeric(sub("\\((.*)\\)","\\1",selframe$bootstrap))
    selframe.duplo<-selframe[duplicated(selframe$taxon)|duplicated(selframe$taxon,fromLast=T),] #zeer elegante oplossing!
    selframe.duplo.select<-selframe.duplo[selframe.duplo$bootstrap>tresh,]
    treshedmulttax<-levels(as.factor(as.character(selframe.duplo.select[duplicated(selframe.duplo.select$taxon),]$taxon)))
    ...
    for(i in 1:length(treshedmulttax))
    {
      ...
      {
        shared.treshed.sum[i,]<-shared.treshed.sum[i,]+colSums(shared.tax[which(as.character(shared.tax$seltax.val)==as.character(corresp[j])),1:ncol(shared.tax)-1])
      }
    }
    ...
    names(shared.treshed.sum)<-names(shared.tax.single) #make sure that names are the same for rbind command
  ...
  if(!is.null(dim(othertax)))
  {
    ...
    othertax <- othertax %>% dplyr::group_by(classif) %>% dplyr::summarise_each(funs(sum)) %>%
